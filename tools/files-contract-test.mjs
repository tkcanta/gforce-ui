...
try {
  const fixtures = [
    ...
    ['native-select', '<label for="native">権限</label><select id="native"><option>閲覧者</option></select>', 'GFU028'],
    ['legacy-select', '<div class="gfu-select-wrap"><select aria-label="権限" class="gfu-field__select"><option>閲覧者</option></select><span data-icon="expand_more"></span></div>', 'GFU028'],
    ['wrapped-arrow', '<button type="button" class="gfu-dropdown__trigger">閲覧者<br><span class="gfu-dropdown__arrow" data-icon="expand_more"></span></button>', 'GFU029'],
